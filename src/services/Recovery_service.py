...
class RecoveryService:
    ...
    def request_password_recovery(email: str):
        ...
        if not user:
            return ApiResponse(message='User not found', code=404).to_response()
        ...
        try:
            ...
            return ApiResponse(message='Recovery code sent successfully', code=200).to_response()
        except Exception as e:
            return ApiResponse(message='Failed to send recovery email', code=500, data=str(e)).to_response()
    ...
    def verify_recovery_code(email: str, recovery_code: str):
        ...
        if not password_recovery or password_recovery.recovery_code != recovery_code:
            return ApiResponse(message='Invalid recovery code or email', code=400).to_response()

        return ApiResponse(message='Recovery code verified', code=200).to_response()
    ...
    def set_new_password(email: str, new_password: str):
        ...
        if not password_recovery:
            return ApiResponse(message='Recovery session not found', code=400).to_response()

        hashed_password = bcrypt.hashpw(new_password.encode('utf-8'), bcrypt.gensalt()).decode('utf-8')
        user_repo.update_password(email, hashed_password)
        recovery_repo.delete_by_email(email)
        return ApiResponse(message='Password updated successfully', code=200).to_response()